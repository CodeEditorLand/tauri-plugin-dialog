	...
	let res = if options.directory {
		#[cfg(desktop)]
		{
			if options.multiple {
				...
				OpenResponse::Folders(folders.map(|folders| {
					folders.iter().map(|p| dunce::simplified(p).to_path_buf()).collect()
				}))
			} else {
		...
		if let Some(files) = &files {
			for file in files {
				...
				window.state::<tauri::scope::Scopes>().allow_file(&file.path)?;
			}
		}
...
pub(crate) async fn message<R: Runtime>(
	...
	Ok(message_dialog(window, dialog, title, message, kind, ok_button_label, None))
}
