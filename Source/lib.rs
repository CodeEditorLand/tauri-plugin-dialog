...
impl<R:Runtime, T:Manager<R>> crate::DialogExt<R> for T {
	fn dialog(&self) -> &Dialog<R> { self.state::<Dialog<R>>().inner() }
}

impl<R:Runtime> Dialog<R> {
	/// Create a new messaging dialog builder.
	/// The dialog can optionally ask the user for confirmation or include an OK
	/// button.
	///
	/// # Examples
	///
	...
	///
	/// The dialog builder includes two separate APIs for rendering the dialog:
	/// `show` and `blocking_show`. The `show` function is asynchronous and
	/// takes a closure to be executed when the dialog is closed. To block the
	/// current thread until the user acted on the dialog, you can use
	/// `blocking_show`, but note that it cannot be executed on the main thread
	/// as it will freeze your application.
	///
	/// ```
	/// use tauri_plugin_dialog::{DialogExt, MessageDialogButtons};
	///
...
impl<R:Runtime> MessageDialogBuilder<R> {
	...
	pub fn blocking_show(self) -> bool { blocking_fn!(self, show) }
}
...
impl<R:Runtime> FileDialogBuilder<R> {
	...
	/// Set whether it should be possible to create new directories in the
	/// dialog. Enabled by default. **macOS only**.
	...
	/// # Reading the files
	///
	/// The file paths cannot be read directly on Android as they are behind a
	/// content URI. The recommended way to read the files is using the [`fs`](https://v2.tauri.app/plugin/file-system/) plugin:
	///
	/// ```
	/// use tauri_plugin_dialog::DialogExt;
